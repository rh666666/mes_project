  ...
  <view class="page">
    ...
    <view class="filter-section">
      ...
      <view v-if="!isLoading" class="results-stats">
        <text class="stats-text">共 {{ deviceSkillList.length }} 条绑定记录</text>
    ...
    >
      ...
      <wd-status-tip v-if="deviceSkillList.length === 0 && !isLoading" image="search" tip="暂无绑定记录" />
    </scroll-view>
    ...
    <wd-popup v-model="showAddDialog" position="bottom" :safe-area-inset-bottom="true">
      <view class="popup-content">
        <view class="popup-header">
          <text class="popup-title">添加设备技能绑定</text>
          <wd-icon name="close" size="20" @click="showAddDialog = false" />
        </view>
        <view class="popup-body">
          <!-- 设备选择 - 使用 SearchableSelector 组件 -->
          <SearchableSelector
...
/**
 * 设备技能绑定管理页面
 * @description 提供设备技能绑定列表查看、添加和删除功能
 */
export default {
  components: {
    SearchableSelector
  ...
  computed: {
    ...
    loadMoreState() {
      ...
      if (!this.hasMore && this.deviceSkillList.length > 0) return 'finished'
      return 'default'
    }
  },

  onLoad() {
    this.loadDeviceSkillList()
    this.loadSkillList()
  },
  ...
  methods: {
     ...
     */
    async loadDeviceSkillList(isLoadMore = false) {
      if (isLoadMore) {
      ...
      try {
        ...
        const res = await skillApi.getDeviceSkillList(params)
        if (res.code === 2000) {
          const newData = res.data || []
          this.total = res.total || 0

          if (isLoadMore) {
            this.deviceSkillList = [...this.deviceSkillList, ...newData]
          } else {
            this.deviceSkillList = newData
          }

          this.hasMore = this.deviceSkillList.length < this.total
        } else {
          uni.showToast({
            title: res.msg || '获取设备技能列表失败',
            icon: 'none'
          })
        }
      } catch (error) {
        console.error('获取设备技能列表失败:', error)
        uni.showToast({
          title: error.msg || '获取设备技能列表失败',
          icon: 'none'
        })
      } finally {
    ...
    async loadSkillList() {
      try {
        const res = await skillApi.getSkillList({ page: 1, limit: 100, type: 'device' })
        if (res.code === 2000) {
     ...
     * @returns {Promise}
     */
    async fetchDeviceSkills(params) {
      const res = await skillApi.getSkillList({ ...params, type: 'device' })
      return res
    },
    ...
    onRefresh() {
      ...
      this.currentPage = 1
      this.hasMore = true
      this.loadDeviceSkillList(false)
    },
    ...
    async onLoadMore() {
      ...
      this.currentPage++
      await this.loadDeviceSkillList(true)
    },
    ...
    filterByKeyword() {
      ...
      this.deviceSkillList = this.deviceSkillList.filter(item =>
    ...
    async onAddBind() {
      ...
      this.isAdding = true
      try {
        const res = await skillApi.createDeviceSkill({
          device: this.addForm.device,
        ...
        console.error('添加设备技能绑定失败:', error)
        uni.showToast({
    ...
    async onConfirmDelete(id) {
      ...
      try {
        ...
        console.error('删除设备技能绑定失败:', error)
        uni.showToast({
