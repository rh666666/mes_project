  ...
  <view class="page">
    ...
    <view class="filter-section">
      ...
      <view v-if="!isLoading" class="results-stats">
        <text class="stats-text">共 {{ processSkillList.length }} 条绑定记录</text>
    ...
    >
      <wd-cell-group>
        <wd-swipe-action
          v-for="(item, index) in processSkillList"
          :key="item.id"
        ...
        >
          <wd-cell
            ...
            :label="`${item.process_code} · ${item.skill_code} - ${item.skill_name}`"
            clickable
      ...
      <wd-status-tip v-if="processSkillList.length === 0 && !isLoading" image="search" tip="暂无绑定记录" />
    </scroll-view>
    ...
    <wd-popup v-model="showAddDialog" position="bottom" :safe-area-inset-bottom="true">
      <view class="popup-content">
        <view class="popup-header">
          <text class="popup-title">添加工序技能绑定</text>
          <wd-icon name="close" size="20" @click="showAddDialog = false" />
        </view>
        <view class="popup-body">
          <!-- 工序选择 - 使用 SearchableSelector 组件 -->
          <SearchableSelector
...
/**
 * 工序技能绑定管理页面
 * @description 提供工序技能绑定列表查看、添加和删除功能
 */
export default {
  components: {
    SearchableSelector
  ...
  computed: {
    ...
    loadMoreState() {
      ...
      if (!this.hasMore && this.processSkillList.length > 0) return 'finished'
      return 'default'
    }
  },

  onLoad() {
    this.loadProcessSkillList()
  },
  ...
  methods: {
     ...
     */
    async loadProcessSkillList(isLoadMore = false) {
      if (isLoadMore) {
      ...
      try {
        ...
        const res = await processApi.getProcessSkillList(params)
        if (res.code === 2000) {
          const newData = res.data || []
          this.total = res.total || 0

          if (isLoadMore) {
            this.processSkillList = [...this.processSkillList, ...newData]
          } else {
            this.processSkillList = newData
          }

          this.hasMore = this.processSkillList.length < this.total
        } else {
          uni.showToast({
            title: res.msg || '获取绑定列表失败',
            icon: 'none'
          })
        }
      } catch (error) {
        console.error('获取工序技能绑定列表失败:', error)
        uni.showToast({
          title: error.msg || '获取绑定列表失败',
          icon: 'none'
        })
      } finally {
    ...
    onRefresh() {
      ...
      this.currentPage = 1
      this.hasMore = true
      this.loadProcessSkillList(false)
    },
    ...
    async onLoadMore() {
      ...
      this.currentPage++
      await this.loadProcessSkillList(true)
    },
    ...
    filterByKeyword() {
      ...
      this.processSkillList = this.processSkillList.filter(item =>
    ...
    /**
     * 工序选择回调
     * @param {Object} process - 选中的工序
     */
    onProcessSelect(process) {
      console.log('选中工序:', process)
    },
    ...
    async onAddBind() {
      ...
      this.isAdding = true
      try {
        const res = await processApi.createProcessSkill({
        ...
        console.error('添加工序技能绑定失败:', error)
        uni.showToast({
    ...
    onDelete(item) {
      uni.showModal({
        title: '确认删除',
        content: `确定要删除工序 "${item.process_name}" 的技能 "${item.skill_name}" 绑定吗？`,
        confirmText: '删除',
        confirmColor: '#ee0a24',
        success: (res) => {
          if (res.confirm) {
    ...
    async onConfirmDelete(id) {
      ...
      try {
        ...
        console.error('删除工序技能绑定失败:', error)
        uni.showToast({
